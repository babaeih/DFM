...
print(factor_struct)
dt_range = pd.date_range(dt.datetime(year=2023,month=7,day=31), dt.datetime(year=2023,month=11,day=30), freq='M')

variables = ['GDPC1']
strt=data.index[0]
dt_range= pd.date_range(dt.datetime(year=2018,month=7,day=31), dt.datetime(year=2024,month=7,day=31), freq='Q')
...
for i in range(0,len(dt_range)-1,1):         
    data1=data.loc[data.index[0]:dt_range[i]]   

    mod=DynamicFactorMQ(endog=data1,k_endog_monthly=k_endog_monthly_,factors=factor_struct,factor_orders=lags,factor_multiplicities={factor_name: 3},idiosyncratic_ar1=True,standardize=False)
    res=mod.fit()
    print(res.summary())
    prediction_results = res.get_prediction(start=data.index[0], end=dt_range[i+1])
    point_predictions = prediction_results.predicted_mean[variables]
    predict.loc[dt_range[i+1],variables]=point_predictions[variables].values[-1][0]
    ...
    with sns.color_palette('deep'):
        ...
        fig.tight_layout()
plt.plot(predict,color='green',linestyle='dotted') 
plt.show()
